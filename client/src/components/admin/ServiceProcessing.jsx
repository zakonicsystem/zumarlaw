  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const statsRes = await axios.get(`${apiUrl}/api/admin/service-status-counts`);
        setServiceStats(statsRes.data);
        // Fetch only latest 4 completed services from all models
        const completedRes = await axios.get(`${apiUrl}/api/admin/latest-completed-services?limit=4`);
