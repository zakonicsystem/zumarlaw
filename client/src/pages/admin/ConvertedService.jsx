...
const ConvertedService = () => {
  ...
  const handleProgressChange = async (row, newProgress) => {
    try {
      await axios.patch(`http://localhost:5000/convertedService/${row._id}/progress`, { progressStatus: newProgress });
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedRows(filtered.map(row => row._id));
      setSelectAll(true);
    }
  };
 ...
 <button
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="mt-2">
                      <select
                        ...
                        className={`text-xs rounded px-2 py-1 ${getProgressClass(row.progressStatus)}`}
                      >
        ...
        {/* Pagination Controls */}
        {/* Pagination Controls Removed: All rows shown on one page */}
